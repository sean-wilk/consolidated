...
if ( ! class_exists( 'Go_Plugin_Manager' ) ) {
	...
	class Go_Plugin_Manager {
		...
		private function set_plugins( $plugins ) {
			...
			/**
			 * Include Go_Plugins class to store
			 * plugin data.
			 */
			require_once( $this->package['directory'] . '/class-go-plugins.php' );

			// Setup plugins.
			$this->plugins = new Go_Plugins( $plugins, $this );
		...
		private function set_args( $args = array() ) {
			...
			$this->args = wp_parse_args( $args, array(
				...
				'views_title'          => sprintf( __( 'Suggested by %s', 'go' ), $this->package['name'] ),
				// translators: 1: name of theme
				'tab_title'            => sprintf( __( 'Suggested by %s', 'go' ), $this->package['name'] ),
				// translators: 1: name of theme
				'extended_title'       => sprintf( __( 'Suggested Plugins by %s', 'go' ), $this->package['name'] ),
				'menu_title'           => '', // Takes on page_title, when left blank.
				'parent_slug'          => 'themes.php',
				'menu_slug'            => 'suggested-plugins',
				'capability'           => 'install_plugins',
				'nag_action'           => __( 'Manage suggested plugins', 'go' ),
				'nag_dismiss'          => __( 'Dismiss this notice', 'go' ),
				// translators: 1: name of theme
				'nag_update'           => __( 'Not all of your active, suggested plugins are compatible with %s.', 'go' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_single'   => __( '%1$s suggests installing %2$s plugin.', 'go' ),
				// translators: 1: name of theme, 2: number of suggested plugins
				'nag_install_multiple' => __( '%1$s suggests installing %2$s plugins.', 'go' ),
				'child_theme'          => false,
				'plugin_file'          => '',
			));
		...
		private function set_notices() {
			...
			/**
			 * Include Go_Plugin_Notices class to setup
			 * and display admin-wide notices, if they're needed.
			 */
			require_once( $this->package['directory'] . '/class-go-plugin-notices.php' );
			...
			$notices = new Go_Plugin_Notices( $args, $this, $this->plugins );
		...
		public function add_assets() {
			...
			wp_localize_script(
				...
				array(
					'thirdParty'   => __( 'Only plugins from wordpress.org can be installed directly here.', 'go' ),
					'notInstalled' => __( 'Plugin update skipped because it is not installed.', 'go' ),
				)
			);
		...
		public function display_page() {
			...
			<div id="suggested-plugins" class="wrap">
				...
				<?php if ( $plugins ) : ?>
					...
					<form method="post" id="bulk-action-form" data-namespace="go">
				...
				<?php else : ?>
					...
					<p><?php esc_html_e( 'No suggested plugins given.', 'go' ); ?></p>
		...
		private function display_table_nav( $which ) {
			...
			$actions = array(
				'install-selected'    => __( 'Install', 'go' ),
				'activate-selected'   => __( 'Activate', 'go' ),
				'deactivate-selected' => __( 'Deactivate', 'go' ),
				'update-selected'     => __( 'Update', 'go' ),
				'delete-selected'     => __( 'Delete', 'go' ),
			);
			...
			<div class="tablenav">
				<div class="actions bulkactions">
					...
					<label for="bulk-action-selector-<?php echo esc_attr( $which ); ?>" class="screen-reader-text">
						<?php esc_html_e( 'Select bulk action', 'go' ); ?>
					</label>

					<select name="action-<?php echo $which; ?>" id="bulk-action-selector-<?php echo esc_attr( $which ); ?>">

						<option value="-1"><?php esc_html_e( 'Bulk Actions', 'go' ); ?></option>
					...
					<?php
					submit_button( __( 'Apply', 'go' ), 'action', '', false,
						array(
							'id' => "do-action-$which",
						)
					);
			...
			<tr class="<?php echo esc_attr( $class ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-plugin="<?php echo esc_attr( $plugin['file'] ); ?>" data-status="<?php echo esc_attr( $plugin['status'] ); ?>" data-source="<?php echo esc_attr( $this->get_plugin_source( $plugin ) ); ?>">
				...
				<th scope="row" class="check-column">
					<label class="screen-reader-text" for="">
						...
						printf( __( 'Select %s', 'go' ), $plugin['name'] );
						?>
					</label>
				...
				<td class="column-installed-version">
					...
					<?php else : ?>
						<?php esc_html_e( 'Not Installed', 'go' ); ?>
					<?php endif; ?>
				</td>
		...
		private function display_actions( $plugin ) {
			...
			if ( $is_wp || 'not-installed' !== $plugin['status'] ) {
				...
				$actions['details'] = array(
					...
					'label'  => sprintf( __( 'More Information about %s', 'go' ), $plugin['name'] ),
					'target' => '_blank',
					'nonce'  => null,
				);
			...
			if ( 'not-installed' === $plugin['status'] ) {
				...
				if ( $is_wp ) {
					...
					$actions['install'] = array(
						...
						'text'   => __( 'Install', 'go' ),
						// translators: 1: name of plugin
						'label'  => sprintf( __( 'Install %s', 'go' ), $plugin['name'] ),
						'target' => '_self',
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['activate'] = array(
					...
					'text'   => __( 'Activate', 'go' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Activate %s', 'go' ), $plugin['name'] ),
					'target' => '_self',
				);
				...
				$actions['deactivate'] = array(
					'url'    => $url,
					'text'   => __( 'Deactivate', 'go' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Deactivate %s', 'go' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( 'inactive' === $plugin['status'] ) {
				...
				$actions['delete'] = array(
					...
					'text'   => __( 'Delete', 'go' ),
					// translators: 1: name of plugin
					'label'  => sprintf( __( 'Delete %s', 'go' ), $plugin['name'] ),
					'target' => '_self',
			...
			if ( $plugin['update'] ) {
				...
				$actions['update'] = array(
					...
					'label'  => sprintf(
						...
						__( 'Update %1$s to version %2$s', 'go' ),
						$plugin['name'],
						$plugin['new_version']
					),
					'target' => '_self',
				...
				$actions['update'] = array(
					'text' => __( 'Plugin is up-to-date', 'go' ),
				);
		...
		private function display_status( $plugin ) {
			...
			switch ( $plugin['status'] ) {
				case 'active':
					esc_html_e( 'Active', 'go' );
					break;

				case 'incompatible':
					esc_html_e( 'Incompatible', 'go' );
					break;

				case 'inactive':
					esc_html_e( 'Installed', 'go' );
					break;

				default:
					esc_html_e( 'Not Installed', 'go' );
			}
		...
		public function row_refresh() {
			 ...
			 * object is used.
			 */
			if ( empty( $_POST['namespace'] ) || 'go' !== $_POST['namespace'] ) {
				return;
			}
		...
		public function request() {
			...
			if ( isset( $_REQUEST['success'] ) ) {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					$message = __( 'Plugin activated.', 'go' );
					...
					$message = __( 'Plugin deactivated.', 'go' );

				} elseif ( 'deactivate-selected' === $_REQUEST['action'] ) {

					$message = __( 'Plugins deactivated.', 'go' );
			...
			if ( $do_bulk && empty( $_REQUEST['checked'] ) ) {
				...
				$error = __( 'No plugins were selected.', 'go' );

			} elseif ( ! $do_bulk && empty( $_REQUEST['plugin'] ) ) {

				$error = __( 'No plugin slug was given.', 'go' );

			} elseif ( ! current_user_can( 'update_plugins' ) ) {

				$error = __( 'Sorry, you are not allowed to update plugins for this site.', 'go' );

			} elseif ( ! $do_bulk && empty( $plugins_data[ $_REQUEST['plugin'] ] ) ) {

				$error = sprintf(
					// translators: 1: slug of plugin being activated
					__( 'The plugin %s doesn\'t exist within the plugin manager\'s registered plugins.', 'go' ),
					$_REQUEST['plugin']
				);
			...
			if ( ! $error ) {
				...
				if ( $do_bulk ) {
					...
					if ( ! $plugins ) {
						$error = __( 'No valid plugins given for bulk action.', 'go' );
					}
				} else {
				...
				if ( 'activate' === $_REQUEST['action'] ) {
					...
					if ( $success ) { // At least one plugin needed to be successful.
						...
						$error = __( 'None of the selected plugins could be activated. Make sure they are installed.', 'go' );
